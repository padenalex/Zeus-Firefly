...
public class FFQATruck
  ...
  public boolean checkDistanceConstraint(FFQATruck truck) {
    ...
    if (isDiagnostic) {
      ...
      for (int i = 0; i < getNodes().size(); i++) {
        node1 = (FFQANode) getNodes().elementAt(i);
    ...
    /*node2 = (FFQANode) getNodes().elementAt(0);
    distance = (float) Math.sqrt( (double) (node1.getX() - node2.getX())
                                 * (node1.getX() - node2.getX()) +
                                 ( (node1.getY() - node2.getY())
                                  * (node1.getY() - node2.getY())));
  ...
  public boolean checkCapacityConstraint(FFQATruck truck) {
    boolean isDiagnostic = true;
    boolean status = true;
    ...
    if (isDiagnostic) {
      ...
      System.out.println("Sequence of the route is:");
      for (int i = 0; i < getNodes().size(); i++) {
        node = (FFQANode) getNodes().elementAt(i);

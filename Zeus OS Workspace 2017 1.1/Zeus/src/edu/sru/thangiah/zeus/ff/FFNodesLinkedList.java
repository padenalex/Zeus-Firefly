...
import edu.sru.thangiah.zeus.ff.FFNodes;
...
public class FFNodesLinkedList
   ...
   */
  public FFNodesLinkedList(FFTruckType tT, double depotX, double depotY, int tN) {

	  //super(tT, depotX, depotY, tN);
	  setTruckType(tT);
	  setTruckNum(tN);
	  //Set the feasibility check to be done for the route
	  setFeasibility(new FFFeasibility(getTruckType().getMaxDuration(),
			  getTruckType().getMaxCapacity(), this));
	  setHead(new FFNodes(new FFShipment(0, depotX, depotY, 0, 0, "D", "D")));
	  setTail(new FFNodes(new FFShipment( -1, depotX, depotY, 0, 0, "D", "D")));
	  linkHeadTail();
	  //assign the FF attributes
	  setAttributes(new FFAttributes());
  }
   ...
   * @return first cell
   */
  public FFNodes getFFHead() {
    return (FFNodes) getHead();
  }
  ...
  public boolean getInsertShipment(FFNodesLinkedList currNodeLL,
                                   FFShipment theShipment) {
    return false;
  }
  ...
  public boolean insertShipment(Shipment theShipment) {
    ...
    FFNodesLinkedList status = (FFNodesLinkedList) ProblemInfo.insertShipType;
    return status.getInsertShipment(this, (FFShipment) theShipment);
  }
  ...
  /**
   * This was in the LinearGreedyInsertShipment class and was move to the FFNodesLinkedList class
   * @return Object
   */
  public Object clone() {

    FFNodesLinkedList clonedNodesLinkedList = new FFNodesLinkedList();
    ...
    if (this.getFeasibility() != null)
    	clonedNodesLinkedList.setFeasibility( (FFFeasibility)this.getFeasibility().clone());
    else
    	clonedNodesLinkedList.setFeasibility(new FFFeasibility());
    
    if (this.getTruckType() != null)
    	clonedNodesLinkedList.setTruckType( (FFTruckType)this.getTruckType().clone());
    else
    	clonedNodesLinkedList.setTruckType(new FFTruckType());

    clonedNodesLinkedList.setTruckNum(this.getTruckNum());
    clonedNodesLinkedList.setHead( (FFNodes)this.getHead().clone());

    this.expandRoute();

    if (this.getHead() != this.getTail()) {
      FFNodes currentNodes = (FFNodes) clonedNodesLinkedList.getHead();
      FFNodes nextNodes = (FFNodes)this.getHead().getNext();

      while (nextNodes != null) {
        currentNodes.setNext( (FFNodes) nextNodes.clone()); //create the next depot
        currentNodes.getNext().setPrev(currentNodes); //set the next depot's prev
        currentNodes = (FFNodes) currentNodes.getNext();
        nextNodes = (FFNodes) nextNodes.getNext();
...
class LinearGreedyInsertShipment
  ...
  public boolean getInsertShipment(FFNodesLinkedList currNodeLL,
    ...
    FFNodes theCell = new FFNodes(theShipment);
    ...
    if (currNodeLL.getHead().getNext() == currNodeLL.getTail()) {
      ...
      /*{
    	  System.out.println("Printing FFNodesLinkedList");
    	  FFNodes tempPtr= (FFNodes)currNodeLL.getHead();
    	  while (tempPtr != (FFNodes)currNodeLL.getTail())
    	  {
    		  System.out.println("Node number and Demand is:"+tempPtr.getIndex()+" "+tempPtr.getDemand());
    		  tempPtr = tempPtr.getFFNext();
    	  }
      ...
      if (!currNodeLL.getFeasibility().isFeasible()) {
    	...
    	tmpPtr = (FFNodes) currNodeLL.getHead().getNext();
    	tmpPtr.setNext(null);
    	tmpPtr.setPrev(null);
        ...
        //print out current route
        /*{
      	  System.out.println("Printing Undo FFNodesLinkedList");
      	  FFNodes tempPtr= (FFNodes)currNodeLL.getHead();
      	  while (tempPtr != (FFNodes)currNodeLL.getTail())
      	  {
      		  System.out.println("Node number and Demand is:"+tempPtr.getIndex()+" "+tempPtr.getDemand());
    ...
    // the route is not empty
      ...
      double cost = Double.MAX_VALUE;
      FFNodes costCell = null; //cell after which the new cell was inserted to achieve cost

      FFNodes prevCell = (FFNodes) currNodeLL.getHead();
      FFNodes nextCell = (FFNodes) currNodeLL.getHead().getNext();
      ...
      while (nextCell != currNodeLL.getTail()) {
        ...
        //set prevCell and nextCell to the next cells in linked list
        prevCell = nextCell;
        nextCell = (FFNodes) prevCell.getNext();
        //print out current route
        /*{
      	  System.out.println("Printing FFNodesLinkedList");
      	  FFNodes tempPtr= (FFNodes)currNodeLL.getHead();

...
import edu.sru.thangiah.zeus.ff.FFNodesLinkedList;

/**
 * Cost functions specific to FFNodesLinkedList level
 * <p>Title: Zeus - A Unified Object Oriented Model for FF's</p>
 * <p>Description: cost functions specific to FF Nodes LinkedList level</p>
 * <p>Copyright: Copyright (c) 2005</p>
 * <p>Company: </p>
 * @author Sam R. Thangiah
 * @version 2.0
 */
public class FFNodesLLCostFunctions
    extends FFAbstractCostFunctions implements
    java.io.Serializable  {

  public double getTotalCost(Object o) {
    FFNodesLinkedList vNodes = (FFNodesLinkedList) o;
    setTotalCost(o);
  ...
  /* public void setTotalCrossRoadPenaltyCost(Object o) {
     ...
     double crossRoadPenaltyCost = 0;
      ...
      matrixNode = SBFFData.distanceMatrix.getDataAt(currentDMI, nextDMI);
                   ...
                   if (matrixNode.imposeCrossRoadPenaltyCost()) {
                       fixedPenalty = SBFFData.pickUpPoints.findPickUpPoint(next.getPickUpPointName()).getCrossRoadCostPenalty() * actualRouteCost;
      ...
      String busTypeNeeded = next.getShipment().getBusTypeNeeded();
                       ...
                       if (busTypeNeeded.equals(SBFFConstants.REGULAR)) {
                           variablePenalty = next.getDemand() * SBFFConstants.R_CROSS_ROAD_COST_PENALTY * actualRouteCost;
      } else if (busTypeNeeded.equals(SBFFConstants.WHEELCHAIR)) {
                           variablePenalty = next.getDemand() * SBFFConstants.W_CROSS_ROAD_COST_PENALTY * actualRouteCost;
      } else if (busTypeNeeded.equals(SBFFConstants.WHEELCHAIR_ELEVATOR)) {
                           variablePenalty = next.getDemand() * SBFFConstants.WE_CROSS_ROAD_COST_PENALTY * actualRouteCost;
      } else if (busTypeNeeded.equals(SBFFConstants.MONITOR)) {
                           variablePenalty = next.getDemand() * SBFFConstants.M_CROSS_ROAD_COST_PENALTY * actualRouteCost;
                       }
   ...
   crossRoadPenaltyCost = crossRoadPenaltyCost + fixedPenalty + variablePenalty;
      ...
      matrixNode = SBFFData.distanceMatrix.getDataAt(lastDMI, nextDMI);
      ...
      String busTypeNeeded = next.getShipment().getBusTypeNeeded();
                       ...
                       //and how many of them there are
                       if (busTypeNeeded.equals(SBFFConstants.REGULAR)) {
   variablePenalty = next.getDemand() * SBFFConstants.R_CROSS_ROAD_COST_PENALTY;
      } else if (busTypeNeeded.equals(SBFFConstants.WHEELCHAIR)) {
   variablePenalty = next.getDemand() * SBFFConstants.W_CROSS_ROAD_COST_PENALTY;
      } else if (busTypeNeeded.equals(SBFFConstants.WHEELCHAIR_ELEVATOR)) {
   variablePenalty = next.getDemand() * SBFFConstants.WE_CROSS_ROAD_COST_PENALTY;
      } else if (busTypeNeeded.equals(SBFFConstants.MONITOR)) {
   variablePenalty = next.getDemand() * SBFFConstants.M_CROSS_ROAD_COST_PENALTY;
                       }
  ...
  /* public void setTotalTurnAroundPenaltyCost(Object o) {
   ...
   matrixNode = SBFFData.distanceMatrix.getDataAt(startIndex, destinationIndex);

            if (startPC.getType() == SBFFConstants.PICK_UP_POINT) {
     if (matrixNode.imposeTurnAroundPenaltyCost(lastTraveledRoadSegment)) {
                    turnAroundPenaltyCost += (SBFFData.pickUpPoints.findPickUpPoint(matrixNode.getStartName()).getTurnAroundCostPenalty() * actualRouteCost);
                }
            }

            if (matrixNode.getFinishType() == SBFFConstants.PICK_UP_POINT) {
